...
def parseKeys([<<"Host: ",host/binary>>|t],websock) do
  ...
  parseKeys(t,websock{host=host1,port=list_to_integer(binary_to_list(port))})
end
...
def parseKeys([_|t],w) when w.callback!==:false do
  f=w.callback
...
#    Lib.trace("Key: ",numbers)
  list_to_integer(:lists.reverse(numbers)) / spaces
end
